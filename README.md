...
- 4 procedural patterns: Ridgeline (jagged mountain silhouettes), Horizon (banded sunset gradients), Flow Field (generative line art), and Dunes (soft warped sand waves)
- 6 curated color palettes built on OKLCH, so light/dark inversion and shading actually look consistent instead of muddy
...
- Optional grain overlay for a bit of texture instead of a flat gradient
